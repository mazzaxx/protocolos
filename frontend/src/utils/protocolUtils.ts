...
export const determineQueueAssignment = (
  ...
  previousAssignedTo?: 'Manual' | 'Deyse' | 'Enzo' | 'Iago' | null
): 'Manual' | 'Deyse' | 'Enzo' | 'Iago' | null => {
  // Se é uma resubmissão (protocolo devolvido sendo reenviado), sempre vai para Fila Manual
  if (isResubmission) {
    return 'Manual';
  }

  // Se é distribuição, sempre vai para a Fila Manual
  if (isDistribution) {
    return 'Manual';
  }

  // Se tem observações, vai para a Fila Manual
  if (data.observations.trim()) {
    return 'Manual';
  }

  // Se é 2º grau, vai para a Fila Manual
  ...
  // Se não se encaixa nos parâmetros do robô, vai para a Fila Manual
  if (!isRobotEligible) {
    return 'Manual';
  }
